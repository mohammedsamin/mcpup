...
func TestAdapterRealWritePreservesUnknownKeys(t *testing.T) {
	...
	if _, ok := doc["preferredModel"]; !ok {
		t.Fatalf("expected preferredModel top-level key to be preserved")
	}
...
func TestAdapterWriteFromEdgeFixture(t *testing.T) {
	...
	if _, ok := doc["preferredModel"]; !ok {
		t.Fatalf("expected preferredModel key to be preserved")
	}
...
func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".roo")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "roocode", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".roo")
