...
func TestAdapterWriteFromEdgeFixture(t *testing.T) {
	...
	if _, ok := doc["workspaceRules"]; !ok {
		t.Fatalf("expected workspaceRules key to be preserved")
	}
...
func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".cursor")
	if err := os.MkdirAll(workspaceDir, 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	workspaceFile := filepath.Join(workspaceDir, "mcp.json")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "cursor", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".cursor")
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		t.Fatalf("mkdir temp fixture dir: %v", err)
	}
	target := filepath.Join(targetDir, "mcp.json")
