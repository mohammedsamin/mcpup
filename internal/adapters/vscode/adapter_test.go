...
func TestAdapterRealWritePreservesUnknownKeys(t *testing.T) {
	...
	if _, ok := doc["inputs"]; !ok {
		t.Fatalf("expected inputs top-level key to be preserved")
	}

	rawServers := map[string]map[string]json.RawMessage{}
	if err := json.Unmarshal(doc["servers"], &rawServers); err != nil {
...
func TestAdapterWriteFromEdgeFixture(t *testing.T) {
	...
	if _, ok := doc["inputs"]; !ok {
		t.Fatalf("expected inputs key to be preserved")
	}
...
func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".vscode")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "vscode", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".vscode")
