...
func TestAdapterRealWritePreservesUnknownKeys(t *testing.T) {
	...
	if _, ok := doc["telemetry"]; !ok {
		t.Fatalf("expected telemetry top-level key to be preserved")
	}
...
func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".amazonq")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "amazonq", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".amazonq")
