...
func TestAdapterRealWritePreservesUnknownKeys(t *testing.T) {
	...
	if _, ok := doc["editor"]; !ok {
		t.Fatalf("expected editor top-level key to be preserved")
	}
...
func TestAdapterWriteFromEdgeFixture(t *testing.T) {
	...
	if _, ok := doc["telemetry"]; !ok {
		t.Fatalf("expected telemetry key to be preserved")
	}
}

func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".codeium", "windsurf")
	if err := os.MkdirAll(workspaceDir, 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	workspaceFile := filepath.Join(workspaceDir, "mcp_config.json")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "windsurf", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".codeium", "windsurf")
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		t.Fatalf("mkdir temp fixture dir: %v", err)
	}
	target := filepath.Join(targetDir, "mcp_config.json")
