...
func TestAdapterRealWritePreservesUnknownKeys(t *testing.T) {
	...
	if _, ok := doc["theme"]; !ok {
		t.Fatalf("expected theme top-level key to be preserved")
	}
...
func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".gemini")
	if err := os.MkdirAll(workspaceDir, 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	workspaceFile := filepath.Join(workspaceDir, "settings.json")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "gemini", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".gemini")
