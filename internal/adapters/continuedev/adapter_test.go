...
func TestAdapterRealWritePreservesUnknownKeys(t *testing.T) {
	...
	if _, ok := doc["schemaVersion"]; !ok {
		t.Fatalf("expected schemaVersion top-level key to be preserved")
	}
...
func TestAdapterWriteFromEdgeFixture(t *testing.T) {
	...
	if _, ok := doc["version"]; !ok {
		t.Fatalf("expected version key to be preserved")
	}
}

func TestDetectPrefersWorkspaceFile(t *testing.T) {
	workspace := t.TempDir()
	workspaceDir := filepath.Join(workspace, ".continue", "mcpServers")
	if err := os.MkdirAll(workspaceDir, 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	workspaceFile := filepath.Join(workspaceDir, "mcpup.json")
...
func copyFixture(t *testing.T, name string) string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "testdata", "fixtures", "continue", name)
	body := mustRead(t, source)

	targetDir := filepath.Join(t.TempDir(), ".continue", "mcpServers")
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		t.Fatalf("mkdir temp fixture dir: %v", err)
	}
	target := filepath.Join(targetDir, "mcpup.json")
